...
#define RLR_ERRORS(X) \
    ...
    X(RLR_ERR_COULD_NOT_FIND_SUITABLE_BACKEND) \
    X(RLR_ERR_FILE_NOT_FOUND) \
    X(RLR_ERR_FILE_NOT_READ_PROPERLY) \
    X(RLR_ERR_CSV_COLUMN_COUNT_MISMATCH) \
    X(RLR_ERR_IMAGE_NOT_LOADED) \
    X(RLR_ERR_BACKEND_SHADER_COMPILATION) \
    X(RLR_ERR_BACKEND_NULL_HANDLE) \
    X(RLR_ERR_BACKEND_NULL_FUNCTION_POINTER) \
    X(RLR_ERR_BACKEND_TEXTURE_SIZES_ARE_DIFFERENT) \
    X(RLR_ERR_MODEL_FAILED_TO_LOAD) \
    X(RLR_ERR_MODEL_PRIMITIVE_NOT_TRIANGLES) \
    X(RLR_ERR_MODEL_NO_POSITION_ATTRIBUTE) \
    X(RLR_ERR_MODEL_ATTRIBUTE_COUNT_ARE_DIFFERENT) \
    X(RLR_ERR_MODEL_IS_NOT_GLB) \
    X(RLR_ERR_MODEL_COULD_NOT_LOAD_ANIMATION) \
    X(RLR_ERR_ANIMATED_MODEL_NEEDS_BOTH_JOINTS_AND_WEIGHTS_OR_NONE) \
